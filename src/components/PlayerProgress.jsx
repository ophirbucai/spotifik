...
import { TrackBar } from "./TrackBar.tsx";
...
export const PlayerProgress = ({ player, songStatus, progress, setProgress }) => {
  ...
  useEffect(() => {
    if (progress !== null) {
      ...
      window.addEventListener("touchend", onDurationChange);
    }
    return () => {
      window.removeEventListener("mouseup", onDurationChange);
      window.removeEventListener("touchend", onDurationChange);
    };
